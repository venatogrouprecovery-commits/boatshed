...
const paths: Record<PremiumIconName, ReactNode> = {
  ...
  broker: <><circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/></>,
  ...
  star: <><path d="m12 2.5 2.9 6 6.6 1-4.8 4.7 1.1 6.6-5.8-3.1-5.8 3.1 1.1-6.6-4.8-4.7 6.6-1L12 2.5Z"/></>
};

export function PremiumIcon({ name, className = '' }: { name: PremiumIconName; className?: string }) {
  return (
    <svg className={className} width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      {paths[name]}
    </svg>
  );
}
...
export function BoatshedLogo({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`brand-lockup ${compact ? 'brand-lockup-compact' : ''}`}>
      <span className="brand-word">BOATSHED</span>
      <span className="brand-wave" aria-hidden="true" />
      {!compact && <span className="brand-subtitle">Marketplace</span>}
    </div>
  );
}
